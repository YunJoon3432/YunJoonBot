            ...
            if guild.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        '음성 채널에 들어가있어야 사용이 가능합니다.  '
                        '음성 채널에 들어간 후에 %입장을 치면 봇이 들어옵니다.' % self.config.command_prefix)
...
#######################################################################################################################
    ...
    async def cmd_재생목록리셋(self, player, channel):
        """
        명령어:
            {command_prefix}재생목록리
    ...
    async def cmd_블랙리스트(self, message, user_mentions, option, something):
        """
        명령어:
            {command_prefix}블랙리스트 추가 | 제거 ] @UserName
    ...
    async def cmd_자동추가(self, player, url=None):
        """
        명령어:
            {command_prefix}자동추가 [url]

        자동으로 실행되는 플레이 리스트에 노래를 추가합니다.
        """
    ...
    async def cmd_봇초대(self, message, server_link=None):
        """
        명령어:
            {command_prefix}봇초대r invite_link

        윤준봇의 초대링크를 생성합니다.(해당 링크를 통하여 다른 서버에도 윤준봇이 추가 가능합니다.)
        """
    ...
    async def cmd_재생(self, message, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
            {command_prefix}play 유튜브 영상 링크
            {command_prefix}play 검색어
            {command_prefix}play 스포티파이 링크
    ...
    async def cmd_재생시간(self, player, channel, guild, message):
        """
        명령어:
            {command_prefix}재생시간
    ...
    async def cmd_입장(self, channel, guild, author, voice_channel):
        ...
        음성 채널에 들어가 있어야 사용이 가능하며, 노래도 *입장 명령어를 사용하신 후에 *재생 명령어를 사용하여 주시기 바랍니다.
        """
    ...
    async def cmd_일시정지(self, player):
        """
        명령어:
            {command_prefix}일시정지
    ...
    async def cmd_다시재생(self, player):
        """
        명령어:
            {command_prefix}다시재생
    ...
    async def cmd_랜덤재생(self, channel, player):
        """
        명령어:
            {command_prefix}랜덤재생
    ...
    async def cmd_리스트삭제(self, player, author):
        """
        명령어:
            {command_prefix}리스트삭제

        플레이리스트에 있던 모든 노래를 삭제합니다.
        """
    ...
    async def cmd_마지막노래삭제(self, user_mentions, message, author, permissions, channel, player, index=None):
        """
        명령어:
            {command_prefix}마지막노래삭제 [# in queue]
    ...
    async def cmd_볼륨(self, message, player, new_volume=None):
        ...
        볼륨의 크기는 1~100 입니다.(이 숫자보다 작거나 클시에는 사용불가)
        """
    ...
    async def cmd_채팅청소(self, message, channel, guild, author, search_range=50):
        """
        명령어:
            {command_prefix}청소 [range]
    ...
    async def cmd_서버정보(self, guild, author, leftover_args, cat='all'):
        """
        명령어:
            {command_prefix}서버정보 [categories]

        서버의 모든 정보를 확인합니다. (DM으로 발송됩니다.)
        """
    ...
    async def cmd_유저정보(self, author, user_mentions, channel, guild, permissions):
        """
        명령어:
            {command_prefix}유저정보 [@user]

        유저의 펄미션 정보를 확인합니다. (*유저정보 @태그)
        """
    ...
    @owner_only
    async def cmd_봇이름변경(self, leftover_args, name):
        """
        명령어:
            {command_prefix}봇이름변경 [바꿀 이름]
    ...
    async def cmd_봇별명변경(self, guild, channel, leftover_args, nick):
        """
        명령어:
            {command_prefix}봇별명변경 [바꿀 별명]
    ...
    @owner_only
    async def cmd_봇프사변경(self, message, url=None):
        """
        명령어:
            {command_prefix}봇프사변경 [프사 링크]
    ...
    async def cmd_퇴장(self, guild):
        ...
        return Response("`{0.name}`방에서 나갔습니다.".format(guild), delete_after=20)
    ...
    async def cmd_봇재시작(self, channel):
        ...
        윤준봇을 재시작합니다. [ 몇초정도 소요됩니다. ]
        """
        await self.safe_send_message(channel, "\N{WAVING HAND SIGN} 현재 윤준봇이 재시작 중입니다. 몇초정도 소요되니 기다려주시기 바랍니다.")
    ...
    async def cmd_봇종료(self, channel):
        """
        명령어:
            {command_prefix}봇종료
        ...
        윤준봇을 종료합니다.
        """
        await self.safe_send_message(channel, "\N{WAVING HAND SIGN}윤준봇을 종료하고 있습니다.")
    ...
    async def cmd_강제퇴장(self, val, leftover_args):
        """
        명령어:
            {command_prefix}강제퇴장 <name/ID>

        해당 서버에서 봇을 강제퇴장 시킵니다.
        """
        if leftover_args:
